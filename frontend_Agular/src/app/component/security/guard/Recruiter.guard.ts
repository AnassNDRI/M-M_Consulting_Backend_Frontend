...
export function RecruiterGuard(
  ...
  console.log('Je suis dans RecruiterGuard');

  // Vérifie si l'utilisateur est déjà authentifié et possède l'un des rôles nécessaires
  if (authService.getIsAuthenticated() && (authService.isRecruiter)) {
    console.log('Accès autorisé pour Recruteur');
    return true; // Accès autorisé pour admin ou consultant
  }

  console.log('Passage dans RecruiterGuard pour vérification supplémentaire');
