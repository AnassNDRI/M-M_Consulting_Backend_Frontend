...
export function CandidateGuard(
  ...
  console.log('Je suis dans CandidateGuard');

  // Vérifie si l'utilisateur est déjà authentifié et possède l'un des rôles nécessaires
  if (authService.getIsAuthenticated() && (authService.isCandidate)) {
    console.log('Accès autorisé Canditat');
    return true; // Accès autorisé pour admin ou consultant
  }

  console.log('Passage dans CandidateGuard pour vérification supplémentaire');
